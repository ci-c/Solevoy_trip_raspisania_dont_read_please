...
class ErrorMonitor:
    ...
    def record_error(
        ...
        if self.error_counts[function_name] > 5:
            ...
            logger.warning(f"Function {function_name} added to blacklist due to repeated errors")
    ...
    def get_recent_errors(self, function_name: str | None = None) -> list[ErrorInfo]:
        ...
        recent_errors = [
            ...
            if now - error.timestamp <= self.time_window
        ]
        
        if function_name:
            return [error for error in recent_errors if error.function_name == function_name]
    ...
    def should_skip_function(self, function_name: str) -> bool:
        ...
        return len(recent_errors) > 3  # Пропускаем если больше 3 ошибок за последние 5 минут
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T | None]:
        ...
        def wrapper(*args, **kwargs) -> T | None:
            ...
            try:
                ...
                error_monitor.record_error(function_name, e, {"args": args, "kwargs": kwargs})
                logger.error(f"{error_message} in {function_name}: {e}")
                if log_traceback:
