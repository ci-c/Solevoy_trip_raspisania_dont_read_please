...
class StringValidator(BaseValidator[str]):
    ...
    def _validate_impl(self, data: Any) -> str:
        ...
        if self.max_length and len(data) > self.max_length:
            if self.level == ValidationLevel.STRICT:
                raise ValidationError(f"String too long: {len(data)} > {self.max_length}")
        ...
        if self.pattern and not self.pattern.match(data):
            raise ValidationError(f"String doesn't match pattern: {self.pattern.pattern}")
    ...
    def _validate_impl(self, data: Any) -> int:
        if isinstance(data, str):
            ...
            try:
                ...
                raise ValidationError(f"Cannot convert {type(data).__name__} to integer")
        ...
        for key, value in data.items():
            if key in self.key_validators:
                ...
                if not result.is_valid:
                    raise ValidationError(f"Validation failed for key '{key}': {result.errors}")
    ...
    def __init__(self, level: ValidationLevel = ValidationLevel.NORMAL):
        super().__init__(
            ...
            key_validators={
                ...
                "subscription_type": StringValidator(pattern=r"^(free|standard|premium)$")
            },
            level=level
        )
