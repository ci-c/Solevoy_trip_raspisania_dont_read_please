...
- Поиск групп по номеру с нормализацией (103а, 204б и т.д.)
- Объединение расписаний лекций и семинаров в единую структуру
- Группировка занятий по дням недели и времени
- Детекция типов расписаний (лекции/семинары/практики)
- Извлечение метаинформации о группах (специальность, курс, поток)
- Форматирование расписания для отображения пользователю
...
from app.schedule.api import search_schedules
...
class UnifiedLesson:
    ...
    def full_info(self) -> str:
        ...
        return (
            f"{type_emoji} {self.subject} ({self.type})\n"
            f"     👨‍🏫 {self.teacher} | 🏢 {self.room}{subgroup_info}"
        )
