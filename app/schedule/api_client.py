...
class APIClient:
    ...
    def _get_schedule_data_sync(self, schedule_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            logger.debug(f"Loaded schedule {schedule_id} with {len(data['scheduleLessonDtoList'])} lessons")
            return data
            ...
            try:
                schedule_ids = await asyncio.wait_for(
                    loop.run_in_executor(
                        ...
                        group_stream, speciality, course_number,
                        academic_year, lesson_type, semester
                    ),
                    timeout=20.0
                )
                return schedule_ids
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            try:
                schedule_data = await asyncio.wait_for(
                    loop.run_in_executor(executor, self._get_schedule_data_sync, schedule_id),
                    timeout=25.0
                )
                return schedule_data
                
            except asyncio.TimeoutError:
                logger.error(f"Async schedule data request timed out for ID {schedule_id}")
                return None
    ...
    async def search_schedules(self, filters: Dict[str, List[str]]) -> List[Dict]:
        ...
        try:
            ...
            if group and not course_number:
                for group_num in group:
                    if group_num and group_num[0].isdigit():
                        ...
                        logger.info(f"Extracted course {course_number[0]} from group {group_num}")
                        break
            ...
            for i, (schedule_id, schedule_data) in enumerate(zip(schedule_ids[:max_schedules], schedule_data_list)):
                if isinstance(schedule_data, Exception):
                    logger.error(f"Error loading schedule {schedule_id}: {schedule_data}")
                    continue
                ...
                if group:
                    ...
                    if not group_found:
                        logger.debug(f"Schedule {schedule_id} doesn't contain requested group")
                        continue
    ...
    def _create_display_name(self, schedule_data: Dict, schedule_id: int) -> str:
        ...
        if lessons:
            ...
            return f"{speciality} - {course_num} курс, {stream} поток, {semester} {year}"
        else:
