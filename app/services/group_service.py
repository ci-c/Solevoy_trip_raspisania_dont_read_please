...
class GroupService:
    ...
    @async_error_handler(default_return=None, error_message="Failed to find or create group")
            ...
            async for session in get_session():
                ...
                if group:
                    ...
                    if not validation_result.is_valid:
                        logger.error(f"Group data validation failed: {validation_result.errors}")
                        return None
                ...
                validation_result = validate_group_data(group_data)
                if not validation_result.is_valid:
                    logger.error(f"New group data validation failed: {validation_result.errors}")
                    return None
        ...
        try:
            async for session in get_session():
                ...
                if existing_group:
                    ...
                    if existing_group.faculty_id != faculty_id or existing_group.speciality_id != speciality_id:
                        existing_group.faculty_id = faculty_id
                        existing_group.speciality_id = speciality_id
                        existing_group.course = course
                        await session.commit()
                        logger.info(f"Updated group {group_name} with faculty {faculty_id}, speciality {speciality_id}")
                ...
                logger.info(f"Created new group {group_name} with faculty {faculty_id}, speciality {speciality_id}")
