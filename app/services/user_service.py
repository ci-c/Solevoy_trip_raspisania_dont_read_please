...
from ..models.user import User, StudentProfile, Subscription, AccessLevel


class UserService:
    """Асинхронный сервис для управления пользователями."""

    async def create_user(self, telegram_id: int, telegram_username: Optional[str] = None, 
        ...
        async with get_db() as conn:
            ...
            """, (user.telegram_id, user.telegram_username, user.full_name, 
                  user.access_level.value, user.last_seen))
    ...
    async def update_user_access_level(self, user_id: int, access_level: AccessLevel) -> bool:
        ...
        async with get_db() as conn:
            ...
            if existing:
                ...
                await conn.execute("""
                    ...
                    WHERE user_id = ?
                """, (profile.group_id, profile.student_id, profile.preferred_format, 
                ...
                cursor = await conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?)
                """, (profile.user_id, profile.group_id, profile.student_id, profile.preferred_format))
    ...
    async def create_subscription(self, subscription: Subscription) -> Subscription:
        ...
        async with get_db() as conn:
            cursor = await conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (subscription.user_id, subscription.plan.value, subscription.started_at,
                  subscription.expires_at, subscription.is_active, subscription.auto_renewal,
                  subscription.payment_method, subscription.subscription_id))
            
            subscription.id = cursor.lastrowid
            await conn.commit()
            
        logger.info(f"Created subscription {subscription.id} for user {subscription.user_id}")
        return subscription
