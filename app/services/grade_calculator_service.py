...
class GradeCalculatorService:
    ...
    async def calculate_knl(self, user_id: int, subject: str) -> float:
        ...
        try:
            ...
            unexcused_absences = sum(
                1 for a in attendance if not a.is_present and not a.is_excused
            )
    ...
    async def get_subject_stats(self, user_id: int, subject: str) -> SubjectStats:
        ...
        try:
            ...
            attended_lessons = sum(
                1 for a in lecture_attendance + seminar_attendance if a.is_present
            )
            excused_absences = sum(
                1
                for a in lecture_attendance + seminar_attendance
                if not a.is_present and a.is_excused
            )
            unexcused_absences = sum(
                1
                for a in lecture_attendance + seminar_attendance
                if not a.is_present and not a.is_excused
            )
    ...
    async def get_user_overall_stats(self, user_id: int) -> Dict[str, str] | None:
        ...
        try:
            ...
            attendance_rate = (
                (total_attended / total_lessons * 100) if total_lessons > 0 else 0.0
            )
