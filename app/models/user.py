"""Модели пользователей и профилей."""

from datetime import datetime
from enum import Enum

from pydantic import Field

from .base import BaseModel


class AccessLevel(str, Enum):
    """Уровни доступа пользователей."""

    GUEST = "guest"
    BASIC = "basic"
    TESTER = "tester"
    ADMIN = "admin"


class SubscriptionPlan(str, Enum):
    """Тарифные планы подписок."""

    FREE = "free"
    STANDARD = "standard"
    PREMIUM = "premium"


class User(BaseModel):
    """Модель пользователя Telegram."""

    id: int | None = Field(None, description="ID в базе данных")
    telegram_id: int = Field(..., description="ID пользователя в Telegram")
    telegram_username: str | None = Field(None, description="Username в Telegram")
    full_name: str | None = Field(None, description="Полное имя пользователя")
    access_level: AccessLevel = Field(AccessLevel.GUEST, description="Уровень доступа")
    is_active: bool = Field(True, description="Активен ли пользователь")
    selected_group_id: int | None = Field(None, description="ID выбранной группы")
    last_seen: datetime | None = Field(
        None, description="Время последней активности",
    )
    created_at: datetime | None = Field(None, description="Время создания")
    updated_at: datetime | None = Field(None, description="Время обновления")


class StudentProfile(BaseModel):
    """Профиль студента."""

    user_id: int = Field(..., description="ID пользователя")
    group_id: int | None = Field(None, description="ID группы студента")
    student_id: str | None = Field(None, description="Номер студенческого билета")
    preferred_format: str = Field("xlsx", description="Предпочитаемый формат файлов")


class Subscription(BaseModel):
    """Подписка пользователя."""

    user_id: int = Field(..., description="ID пользователя")
    plan: SubscriptionPlan = Field(..., description="Тарифный план")
    started_at: datetime = Field(
        default_factory=datetime.now, description="Начало подписки",
    )
    expires_at: datetime | None = Field(None, description="Окончание подписки")
    is_active: bool = Field(True, description="Активна ли подписка")
    auto_renewal: bool = Field(False, description="Автопродление")
    payment_method: str | None = Field(None, description="Способ оплаты")
    subscription_id: str | None = Field(None, description="ID в платежной системе")
