...
class Invitation(BaseModel):
    ...
    access_level: AccessLevel = Field(
        AccessLevel.BASIC, description="Уровень доступа по инвайту",
    )
    ...
    expires_at: datetime | None = Field(None, description="Время истечения инвайта")
    is_active: bool = Field(True, description="Активен ли инвайт")
    metadata: str | None = Field(
        None, description="Дополнительная информация в JSON",
    )
...
class InvitationUsage(BaseModel):
    ...
    used_at: datetime = Field(
        default_factory=datetime.now, description="Время использования",
    )
