...
class ApplicationGenerator:
    ...
    def _generate_discipline_application(
        self, request: AbsenceRequest, subject: str
        ...
        discipline_config = app_config.get("document_types", {}).get(
            "discipline_application", {}
        )

        # Формируем содержимое заявления
        content = self._format_discipline_application(
            request, subject, discipline_config
        )
    ...
    def _generate_explanatory_note(
        self, request: AbsenceRequest
    ) -> GeneratedApplication:
        """Генерирует объяснительную записку в дирекцию."""

        app_config = self.config.get_applications_config()
        explanatory_config = app_config.get("document_types", {}).get(
            "explanatory_note", {}
        )
...
ПРИМЕЧАНИЕ: Документ требует печати, подписи студента и личной подачи в дирекцию института.
    ...
    def _get_additional_info_text(self, request: AbsenceRequest) -> str:
        ...
        app_config = self.config.get_applications_config()
        reason_config = app_config.get("absence_reasons", {}).get(
            request.reason_code, {}
        )
    ...
    def get_document_requirements(self) -> Dict[str, str]:
        ...
        return {
            ...
            "discipline_recipient": workflow.get(
                "discipline_application_recipient", "На кафедру"
            ),
            "explanatory_recipient": workflow.get(
                "explanatory_note_recipient", "Дирекция института"
            ),
        }
