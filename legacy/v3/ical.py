...
def gen_ical(schedule_data: List[PostLesson], subgroup_name: str) -> None:
    ...
    for lesson in schedule_data:
        ...
        try:
            ...
            rich.print(
                f"[bold yellow]Предупреждение:[/bold yellow] Не удалось найти время для '{lesson.subject_name}' ({lesson.date}). Пропускаю."
            )
            continue
        ...
        event.name = (
            f"№{lesson.lesson_number + 1} {lesson.lesson_type} {lesson.subject_name}"
        )
        ...
        event.categories = [
            {"Л": "Лекция", "С": "Семинар"}.get(lesson.lesson_type, "Занятие"),
            "СЗГМУ",
        ]
    ...
    if not calendar.events:
        rich.print(
            f"[bold yellow]Не создано ни одного события. Файл {filename} не будет сохранен.[/bold yellow]"
        )
        return

    rich.print(
        f"Сохраняю файл: [cyan]{filename}[/cyan] ({len(calendar.events)} событий)"
    )
