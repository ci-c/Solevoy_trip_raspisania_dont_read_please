...
def main() -> None:
    ...
    if not all_lessons:
        logger.info(
            "Не найдено занятий для определения даты начала. Завершение работы."
        )
        return

    # 2. Определение первой даты расписания (логика остаётся здесь)
    earliest_lesson = min(
        all_lessons, key=lambda x: (int(x.weekNumber), WEEK_DAYS.get(x.dayName, 999))
    )
    ...
    if not processed_lessons:
        logger.info(
            f"Не найдено занятий для подгруппы {subgroup_name} после обработки. Завершение работы."
        )
        return
