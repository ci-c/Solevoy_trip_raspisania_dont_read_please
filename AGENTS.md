...
- `uv run ruff check .`, `uv run ruff format .`, and `uv run mypy app` keep lint, formatting, and typing aligned with CI expectations.

## Coding Style & Naming Conventions
Use Python 3.10+ with 4-space indentation, `black`-compatible line wrapping, and exhaustive type hints (mypy is set to `disallow_untyped_defs`). Modules and files stay `snake_case`; classes use `PascalCase`; async handlers and services follow verb-based names (`fetch_schedule`, `update_group`). Run `ruff format` before commits, organize imports with `isort`, and verify `flake8`/`ruff check` is clean.
