...
**Performance Goals**: Ответ `/schedule` < 3 с для кэшированных групп; генерация экспорта ≤ 5 с; уведомления доставляются в заданное окно с учётом пользовательских «тихих часов»  
...
- [ ] **Принцип I — Канонические данные расписания**: Подготовить миграции/сидеры под нужные сценарии; убедиться, что новые функции используют `app/services/schedule.py`, импортируют данные с официального сайта (PDF/таблицы) и поддерживают резервное обновление через неофициальный API без прямых чтений файлов в рантайме; нормализовать преподавателей/аудитории через `Lecturer`/`Room`.
...
- [ ] **Принцип III — Ценность для студента**: Пересмотреть копирайтинг и flow, убедиться, что профили хранят текущие настройки тихих часов (без истории) и локализацию; зафиксировать требования в спеце/тестах.
...
**Structure Decision**: Работа затрагивает `app/bot/handlers/` (новые команды/меню), `app/services/` (расписание, экспорт, профиль, уведомления и хранение тихих часов), `app/schedule/` (ингест с официального сайта и резерв из API, экспорт), `app/database/models.py` (изменения для UserProfile, ScheduleLesson, Lecturer, Room, NotificationJob, ExportRequest), а также соответствующие тесты (`tests/unit`, `tests/integration`). Прямой доступ к БД из хендлеров исключается — только через сервисы, что соответствует Принципам I и II.
